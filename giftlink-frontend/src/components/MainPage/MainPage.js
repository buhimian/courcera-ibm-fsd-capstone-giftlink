...
function MainPage() {
    ...
    useEffect(() => {
        ...
        const fetchGifts = async () => {
            ...
            try {
                ...
                // backend may return { docs: [...] } or an array directly
                const giftsArray = Array.isArray(data) ? data : (data.docs || data);
                setGifts(giftsArray || []);
            } catch (error) {
    ...
    return (
        <div className="container mt-5">
            {loading && <div className="text-center">Loading gifts...</div>}
            {error && <div className="alert alert-danger">Error loading gifts: {error}</div>}
            {!loading && !error && gifts.length === 0 && (
                <div className="text-center">No gifts found.</div>
            )}
            <div className="row">
                {gifts.map((gift) => (
